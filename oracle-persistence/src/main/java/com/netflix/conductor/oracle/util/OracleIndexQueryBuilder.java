...
public class OracleIndexQueryBuilder {
    ...
    private static class Condition {
        ...
        public String getQueryFragment() {
            if (operator.equals("IN")) {
                return attribute + " IN (?)"; // return attribute + " = ANY(?)";
            } else if (operator.equals("CONTAINS")) { // Assuming you're using CONTAINS for full-text search
                return "CONTAINS(" + attribute + ", ?, 1) > 0"; // Oracle equivalent of @@ to_tsquery
            } else if (operator.equals("@>")) {
                return attribute + " @> ?"; // JSON support; adjust if needed based on JSON handling
            } else {
                if (attribute.endsWith("_time")) {
                    return attribute + " " + operator + " TO_TIMESTAMP(?)";
                } else {
    ...
    public OracleIndexQueryBuilder(
            String table,
            String query,
            String freeText,
            int start,
    ...
    public String getQuery() {
        ...
        return "SELECT json_data FROM " + table + queryString + getSort() + " OFFSET ? ROWS FETCH NEXT ? ROWS ONLY";
        //return "SELECT json_data::TEXT FROM " + table + queryString + getSort() + " LIMIT ? OFFSET ?";
    }
